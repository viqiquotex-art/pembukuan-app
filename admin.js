...
function getIdentity() {
  try {
    ...
    return { userId, name: localStorage.getItem('cloud_name'), email: localStorage.getItem('cloud_email') };
  } catch { return null; }
}
...
async function loadUsers() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(data.error || (response.status === 403 ? 'Akun ini bukan admin.' : 'Gagal mengambil data pengguna.'));
    ...
    body.innerHTML = users.map(user => `<tr><td><div class="name">${escapeHtml(user.name || 'Tanpa nama')}</div></td><td><div class="email">${escapeHtml(user.email)}</div></td><td>${escapeHtml(formatDate(user.createdAt))}</td><td>${escapeHtml(formatDate(user.lastLogin))}</td><td><span class="badge">Aktif</span></td></tr>`).join('');
  } catch (error) {
