...
function renderUsers() {
  ...
  if (!filtered.length) {
    body.innerHTML = `<tr><td colspan="6" class="empty">${query ? 'Tidak ada pengguna yang cocok.' : 'Belum ada pengguna terdaftar.'}</td></tr>`;
    return;
  }
  body.innerHTML = filtered.map(user => {
    const isAdmin = Boolean(user.isAdmin);
    return `<tr>
      <td><div class="name">${escapeHtml(user.name || 'Tanpa nama')}</div><div class="stat-sub">ID: ${escapeHtml(user.userId || '—')}</div></td>
      <td><div class="email">${escapeHtml(user.email || '—')}</div></td>
      <td>${escapeHtml(formatDate(user.createdAt))}</td>
      <td>${escapeHtml(formatDate(user.lastLogin))}</td>
      <td><span class="badge ${isAdmin ? 'admin' : ''}">${isAdmin ? 'Admin' : 'User'}</span></td>
...
async function loadUsers() {
  ...
  const credentials = getCredentials();
  if (!credentials) {
    document.getElementById('apiStatus').textContent = 'LOCKED';
    document.getElementById('adminIdentity').textContent = 'Login admin diperlukan';
    showStatus('Sesi admin tidak ditemukan. Login menggunakan akun yang terdaftar sebagai ADMIN_EMAIL.', 'error');
  ...
  try {
    ...
    document.getElementById('adminIdentity').textContent = credentials.email || 'Admin terverifikasi';
    showStatus(`Admin terverifikasi • ${allUsers.length} pengguna berhasil dimuat.`, 'success');
  } catch (error) {
    allUsers = [];
    calculateStats([]);
    renderUsers();
    document.getElementById('apiStatus').textContent = 'ERROR';
    showStatus(error.message || 'Terjadi kesalahan saat memuat data.', 'error');
  } finally {
    loading.style.display = 'none';
  }
}
...
async function logout() {
  const credentials = getCredentials();
  if (!credentials) { window.location.href = './cloud.html'; return; }
