...
function getFormTransaction() {
  ...
  if (!typeEl || !categoryEl || !amountEl || !dateEl || !descriptionEl) {
    return { error: '❌ Form transaksi tidak lengkap.' };
  }
...
function addTransaction() {
  ...
  if (typeof isCloudConnected === 'function' && isCloudConnected() && typeof autoSyncToCloud === 'function') {
    autoSyncToCloud();
  }
}
...
function renderHistory() {
  ...
  sorted.forEach(transaction => {
    ...
    item.appendChild(makeText(
      'div',
      `transaction-amount ${isIncome ? 'income' : 'expense'}`,
  ...
  listEl.onclick = function (event) {
    ...
    if (button.dataset.action === 'edit') {
      enterEditMode(id);
    } else if (button.dataset.action === 'delete' && typeof window.deleteTransaction === 'function') {
  ...
  panel.innerHTML = `
    <div class="cloud-info">
      ...
      <div class="cloud-tips">
        ...
        <ul>
          <li>📤 Sync = kirim data lokal</li>
          <li>📥 Muat = ambil data cloud</li>
          <li>🔄 Auto Sync aktif</li>
        </ul>
      </div>
    </div>`;
}
