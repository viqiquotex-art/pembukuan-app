...
function getCloudCredentials() {
  try {
    const data =
      localStorage.getItem('cloud_credentials');
    ...
    const userId =
      localStorage.getItem('cloud_userId');

    const token =
      localStorage.getItem('cloud_token');

    const email =
      localStorage.getItem('cloud_email');

    const name =
      localStorage.getItem('cloud_name');
    ...
    console.error(
      'Failed to read cloud credentials:',
      error
    );
...
function updateCloudStatus() {
  ...
  if (credentials) {
    badge.textContent =
      `☁️ Cloud: ${credentials.name || credentials.email || 'Connected'}`;
      ...
      <div class="transaction-item">
        ...
        <div class="transaction-actions">
          ...
          <button
            ...
            onclick="enterEditMode('${transaction.id}')"
          >
            ✏️ Edit
          </button>
  ...
  if (!credentials) {
    panel.innerHTML = `
      <div class="cloud-info">
        ...
        <h3>
          ☁️ Cloud Storage - Offline
        </h3>

        <p>
          Sinkronisasi data Anda ke cloud
          untuk akses di berbagai device
          dan backup data.
        </p>

        <button
          class="btn btn-primary"
          onclick="goToCloudPage()"
        >
          🔑 Login / Daftar Cloud
        </button>
        ...
        >
          ...
          <ul>
            <li>💾 Backup data</li>
            <li>📱 Akses berbagai device</li>
            <li>🔄 Sinkronisasi</li>
            <li>🔒 Data tersimpan di cloud</li>
          </ul>
        </div>
  ...
  panel.innerHTML = `
    <div class="cloud-info">
      ...
      <div class="cloud-tips">
        ...
        <ul>
          <li>
            📤 Sync = kirim data lokal
          </li>

          <li>
            📥 Muat = ambil data cloud
          </li>
...
async function syncToCloudFromApp() {
  ...
  if (!credentials) {
    showToast(
      '❌ Silakan login cloud terlebih dahulu',
      'error'
    );
    return;
  }
  ...
  try {
    ...
    showToast(
      `✅ Synced! ${data.count} transaksi tersimpan`,
      'success'
    );
...
async function loadFromCloudToApp() {
  ...
  try {
    ...
    const response =
      await fetch(
        `${API_BASE_URL}/api/transactions/${credentials.userId}`,
        {
          method: 'GET',
    ...
    showToast(
      `✅ Cloud berhasil dimuat! ${merged.length} transaksi`,
      'success'
    );
...
function logoutCloud() {
  if (!confirm(
    'Logout dari cloud? Data lokal tetap aman.'
  )) {
    return;
  }
