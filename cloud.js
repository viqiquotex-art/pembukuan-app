...
async function syncToCloud() { const credentials = getCloudCredentials(); if (!credentials) return showAlert('❌ Anda harus login terlebih dahulu', 'error'); if (cloudSyncInFlight) return cloudSyncInFlight; cloudSyncInFlight = (async () => { try { setLoading(true, 'Syncing ke cloud...'); const deletedIds = getLocalDeletedTransactionIds(); const transactions = getDeltaTransactions(parseLocalTransactions(), readSyncSnapshot(credentials.userId), deletedIds); const response = await authFetch(API_ENDPOINTS.saveTransactions, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: credentials.userId, transactions }) }); const data = await safeJson(response); if (response.status === 401) { handleAuthFailure(data.error); return false; } if (response.status === 403) { showAlert('❌ Akses cloud ditolak.', 'error'); return false; } if (!response.ok) { showAlert(`❌ ${data.error || 'Sync gagal'}`, 'error'); return false; } if (Array.isArray(data.deletedIds)) { data.deletedIds.forEach(id => deletedIds.add(id)); saveLocalDeletedTransactionIds(deletedIds); } const serverTransactions = Array.isArray(data.transactions) ? data.transactions : []; const merged = mergeTransactions(parseLocalTransactions(), serverTransactions, deletedIds); localStorage.setItem('transactions', JSON.stringify(merged)); writeSyncSnapshot(credentials.userId, serverTransactions); if (typeof renderHistory === 'function') renderHistory(); if (typeof renderRecap === 'function') renderRecap(); showAlert(`✅ Sync selesai! ${merged.length} transaksi (${transactions.length} perubahan dikirim)`, 'success'); loadStats(); return true; } catch (error) { console.error('Sync error:', error); showAlert(`❌ Sync gagal: ${error.message}`, 'error'); return false; } finally { setLoading(false); cloudSyncInFlight = null; } })(); return cloudSyncInFlight; }
async function loadFromCloud() { const credentials = getCloudCredentials(); if (!credentials) return showAlert('❌ Anda harus login terlebih dahulu', 'error'); try { setLoading(true, 'Memuat dari cloud...'); const response = await authFetch(API_ENDPOINTS.getTransactions(credentials.userId), { method: 'GET' }); const data = await safeJson(response); if (response.status === 401) { handleAuthFailure(data.error); return false; } if (response.status === 403) { showAlert('❌ Akses cloud ditolak.', 'error'); return false; } if (!response.ok) { showAlert(`❌ ${data.error || 'Load gagal'}`, 'error'); return false; } const cloudTransactions = Array.isArray(data.transactions) ? data.transactions : []; const cloudDeletedIds = Array.isArray(data.deletedIds) ? data.deletedIds.filter(Boolean) : []; const localDeletedIds = getLocalDeletedTransactionIds(); cloudDeletedIds.forEach(id => localDeletedIds.add(id)); saveLocalDeletedTransactionIds(localDeletedIds); const merged = mergeTransactions(parseLocalTransactions().filter(t => t?.id && !localDeletedIds.has(t.id)), cloudTransactions.filter(t => t?.id && !localDeletedIds.has(t.id)), localDeletedIds); localStorage.setItem('transactions', JSON.stringify(merged)); writeSyncSnapshot(credentials.userId, cloudTransactions); if (typeof renderHistory === 'function') renderHistory(); if (typeof renderRecap === 'function') renderRecap(); showAlert(`✅ Loaded! ${merged.length} transaksi`, 'success'); loadStats(); return true; } catch (error) { console.error('Load error:', error); showAlert(`❌ Load gagal: ${error.message}`, 'error'); return false; } finally { setLoading(false); } }
async function loadStats() { const credentials = getCloudCredentials(); if (!credentials) return; try { const response = await authFetch(API_ENDPOINTS.getStats(credentials.userId)); const data = await safeJson(response); if (response.status === 401) return handleAuthFailure(data.error); if (!response.ok) return; const stats = data.stats || data; const userName = document.getElementById('userName'); if (userName) userName.textContent = credentials.name || ''; const container = document.getElementById('statsContainer'); if (!container) return; const values = [['💰 Total Pemasukan', formatRupiah(stats.totalIncome)], ['💸 Total Pengeluaran', formatRupiah(stats.totalExpense)], ['📊 Saldo', formatRupiah(stats.balance)], ['📝 Transaksi', Number(stats.transactionCount) || 0]]; const fragment = document.createDocumentFragment(); values.forEach(([label, value]) => { const card = document.createElement('div'); card.className = 'stat-card'; const labelEl = document.createElement('div'); labelEl.className = 'label'; labelEl.textContent = label; const valueEl = document.createElement('div'); valueEl.className = 'value'; valueEl.textContent = value; card.append(labelEl, valueEl); fragment.appendChild(card); }); container.replaceChildren(fragment); } catch (error) { console.error('Failed to load stats:', error); } }
async function validateCloudSession() { const credentials = getCloudCredentials(); if (!credentials) return false; try { const response = await authFetch(API_ENDPOINTS.profile); const data = await safeJson(response); if (response.status === 401) { handleAuthFailure(data.error); return false; } if (!response.ok || !data.userId) return false; saveCloudCredentials({ userId: data.userId, name: data.name, email: data.email, token: credentials.token }); return true; } catch { return false; } }
