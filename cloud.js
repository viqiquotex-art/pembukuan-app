...
function parseLocalTransactions() {
  ...
  } catch (error) { console.warn('Transaction storage error:', error); return []; }
}
function getLocalDeletedTransactionIds() {
  ...
  } catch (error) { console.warn('Deleted transaction storage error:', error); return new Set(); }
}
...
async function loadStats() {
  ...
  try {
    ...
    if (!container) return;
    const values = [['💰 Total Pemasukan', formatRupiah(stats.totalIncome)], ['💸 Total Pengeluaran', formatRupiah(stats.totalExpense)], ['📊 Saldo', formatRupiah(stats.balance)], ['📝 Transaksi', Number(stats.transactionCount) || 0]];
    const fragment = document.createDocumentFragment();
    values.forEach(([label, value]) => { const card = document.createElement('div'); card.className = 'stat-card'; const labelEl = document.createElement('div'); labelEl.className = 'label'; labelEl.textContent = label; const valueEl = document.createElement('div'); valueEl.className = 'value'; valueEl.textContent = value; card.append(labelEl, valueEl); fragment.appendChild(card); });
...
async function validateCloudSession() {
  ...
  try {
    ...
    const data = await safeJson(response);
    if (response.status === 401) { handleAuthFailure(data.error); return false; }
    if (!response.ok || !data.userId) return false;
    saveCloudCredentials({ userId: data.userId, name: data.name, email: data.email });
    return true;
  } catch (error) { console.warn('Session validation failed:', error); return false; }
}

function toggleForm() { const loginForm = document.getElementById('loginForm'); const registerForm = document.getElementById('registerForm'); if (!loginForm || !registerForm) return; if (loginForm.style.display === 'none') { loginForm.style.display = 'block'; registerForm.style.display = 'none'; } else { loginForm.style.display = 'none'; registerForm.style.display = 'block'; } hideAlert(); }
